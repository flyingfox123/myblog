...
This is not exactly what recovery means in this context ¡ª if it's possible to make such recovery as was mentioned in the illustration, then by all means you should do it.
However, recovery could also be displaying an alert dialog to the user that describes the incident, or perhaps sending an email to an administrator, or even simply logging the error to a log file. All of these options qualify as ¡®recovery¡¯ ¨C taking a valid and known course of action in the event of an exception.
...
Examples include situations where you try to connect to a server without an internet connection ¡ª there is no need to crash the app. A gentle way to deal with the error is to display an error dialog that explains what happened, allowing the user to fix their connection, given a clear enough message.
If the error is in the code, and the program itself is defective, then writing a recovery path is irrelevant ¡ª how can you recover from a problem that you don¡¯t even know exists yet? Or if you do know what the problem is, then why write a recovery path at all instead of fixing the problem?
